 ...
 //selection des ville de la base de données  VALUES('$titre','$nbrevictimes','$engins','$nbrenfants','$nbrfemmes','$nbrhommes','$lieu','$ndateEvenement ','$source')");
    $prendre_ville = $pdo->query("SELECT id,ville FROM ville");
     ...
     if(isset($_POST['enregistrement'])) {
        ...
        $insertion = $pdo->query("INSERT INTO vols(titre,nbrevictimes,typevol,nbrenfants,nbrfemmes,nbrhommes,lieu,source,dateEvenement,datepost) 
         VALUES('$titre','$nbrevictimes','$typevol','$nbrenfants','$nbrfemmes','$nbrhommes','$lieu','$source','$dateEvenement',now())");
    
        
        if($insertion){
            echo "<script>
                    alert('Enregistrement effectué avec succès pour continuer')</script>
                    <meta http-equiv=\"refresh\" content=\"0;URL= graph_vols.php\">";
        }
