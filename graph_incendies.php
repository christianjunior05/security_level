    ...
    if (isset($recherche)) {
        if (($lieu != 0) && ($annee != 0)) {
            for ($mois = 1; $mois <= 12; $mois++) { 
                $info = $pdo->prepare("SELECT COUNT(*) AS nbAccident FROM incendies INNER JOIN ville ON lieu = id WHERE MONTH(dateEvenement) = $mois AND (lieu =$lieu) OR (YEAR(dateEvenement) = $annee)");
                $info->execute();
                    ...
                    <div class="menu">
                        <a href="">
                            ...
                            <a href="graph_incendies.php">
                                <div class="puce_verte"> <p style="color:#fff; margin-left: 17px; margin-top: 19px;">06</p> </div> <div class="list-item">Incendies</div>
                            </a>
                            <hr>
