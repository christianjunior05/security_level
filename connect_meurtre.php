<?php 
 ...
 //selection des ville de la base de données  VALUES('$titre','$nbrevictimes','$engins','$nbrenfants','$nbrfemmes','$nbrhommes','$lieu','$ndateEvenement ','$source')");
   ...
   $dateEvenement=$_POST['dateEvenement'];
        ...
        $insertion = $pdo->query("INSERT INTO meurtres_atteintes_physiques(titre,nbrevictimes,typemeurtre,nbrenfants,nbrfemmes,nbrhommes,lieu,source,dateEvenement,datepost) 
         VALUES('$titre','$nbrevictimes','$typemeurtre','$nbrenfants','$nbrfemmes','$nbrhommes','$lieu','$source','$dateEvenement',now())");
    
        
        if($insertion){
            echo "<script>
                    alert('Enregistrement effectué avec succès pour continuer')</script>
                    <meta http-equiv=\"refresh\" content=\"0;URL= graph_meurtre.php\">";
        }
